...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	readLength = alt_avalon_spi_command(SPI_0_BASE, 0, 2, arr, 0, NULL, 0);
	if (readLength < 0){
		alt_printf("Error: Write unsuccessful");
	}
}
//multiple-byte write
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	readLength = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes + 1, arr, 0, NULL, 0);
	if (readLength < 0){
		alt_printf("Error: Write unsuccessful");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int readLength;
	BYTE val;
	readLength = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &val, 0);
	if (readLength < 0){
		alt_printf("Error: Read unsuccessful");
	}
	return val;
}
//multiple-byte write
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	int readLength;
	readLength = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, nbytes, data, 0);
	if (readLength < 0){
		alt_printf("Error: Read unsuccessful");
	}
	return data + nbytes;
